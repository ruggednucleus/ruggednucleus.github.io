...
/*
    for(let y = 0; y < 512; y++) {
        for(let x = 0; x < 512; x++) {
            if(x === 0 && y > 100) {
                ctx.putImageData(imageData, 0, 0);
                return {y: y, x: x, image: imageData, time: performance.now() - dt};
            }

            if(performance.now() - dt > time) {
                ctx.putImageData(imageData, 0, 0);
                return {y: y, x: x, image: imageData};
            }

            let p = Vector(13, 13, 13);
            for(let r = 64; r--;) {
                let t = a.scale(Math.random() - 0.5).scale(99).add(b).scale(Math.random() - 0.5).scale(99);

                p = sampler(Vector(17, 16, 8).add(t),
                            t.scale(-1).add(a.scale(Math.random() + x).add(b).scale(y + Math.random()).add(c)).scale(16).normalise()
                ).scale(3.5).add(p);
            }

            imageData.data[(y * 512 + x) * 4] = p.x() << 0;
            imageData.data[(y * 512 + x) * 4 + 1] = p.y() << 0;
            imageData.data[(y * 512 + x) * 4 + 2] = p.z() << 0;
            imageData.data[(y * 512 + x) * 4 + 3] = 255
        }
    }
*/
}