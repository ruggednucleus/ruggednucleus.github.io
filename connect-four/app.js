    ...
    updateFallingDiscs() {
        if(this.fallingDisc) {
            ...
            if(this.fallingDisc.y >= this.fallingDisc.endY) {
                ...
                const aiNext = this.fallingDisc.aiNext;
                const color = this.fallingDisc.color;
                this.fallingDisc = undefined;
                if(aiNext) {
                    if(color === "red") {
                        this.aiMoveMin(this.depth);
                    } else {
                        this.aiMoveMax(this.depth);
                    }
                }
            }
        }
...
let cellSize = Math.min(window.innerWidth / 7, window.innerHeight / 6) * 0.9 | 0;
